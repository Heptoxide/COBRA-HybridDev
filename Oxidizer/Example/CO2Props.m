function [Props] = CO2Props(T,rho)
 ...
 % dioxide (CO2) for a given density (kg/mˆ3) and temperature (K). Valid
 ...
 % d = rho/rhoc; % Reduced density,  = phi/phi critical, phi ? mass ...
density
 ...
 %??????????????????????????????????????????????????????????????????????????
 % P melt = Pt* (1 + a m(1)*(T/Tt?1) + a m(2)*(T/Tt?1)ˆ2); % Melting ...
% Pressure, Eq. 3.10
...
0000 3.0000 6.0000 3.0000 6.0000 8.0000 6.0000 0.0000 7.0000 12.0000 ...
0000 22.0000 24.0000 16.0000 24.0000 8.0000 2.0000 28.0000 14.0000]';
...
55068668612842E?1];
 ...
 %??????????????????????????????????????????????????????????????????????????
 tta = (1?t) + A42 .* ((d?1).ˆ2).ˆ(0.5./bt42);
 Del = tta.ˆ2 + B42.*((d?1).ˆ2).ˆa42;
 Psi = exp(?C42.*(d?1).ˆ2 ? D42.*(t?1).ˆ2);

 Psi d = ?2 .* C42 .* (d?1) .* Psi;
 Psi t = ?2 .* D42 .* (t?1) .* Psi;
 Del d = (d?1).*(A42 .* tta .* 2./bt42 .* ...
 ((d?1).ˆ2).ˆ(.5./bt42?1) + ...
 .* B42 .* a42 .* ((d?1).ˆ2).ˆ(a42?1));

 Psi dd = 2.*C42.*Psi.*(2.*C42.*(d?1).ˆ2 ? 1);
 Psi tt = 2.*D42.*Psi.*(2.*D42.*(t?1).ˆ2 ? 1);
 Del dd = 1./(d?1).*Del d + (d?1).ˆ2 ...
 .* (4.*B42.*a42.*(a42?1).*((d?1).ˆ2).ˆ(a42?2) + ...
*A42.ˆ2.*(1./bt42).ˆ2.*(((d?1).ˆ2).ˆ(1./(2.*bt42)?1)).ˆ2 + ...
 A42.*tta.*4./bt42.*(.5./bt42?1).*((d?1).ˆ2).ˆ(.5./bt42?2));

 Psi dt = 4.*C42.*D42.*(d?1).*(t?1).*Psi;

 % Residual part of the Helmholtz energy, Eq. 6.5
 %??????????????????????????????????????????????????????????????????????????
 phir = sum(n7 .* d.ˆd7 .* t.ˆt7) ...
 + sum(n34 .* d.ˆd34 .* t.ˆt34 .* exp(?d.ˆc34)) ...
 + sum(n39 .* d.ˆd39 .* t.ˆt39 .* ...
 exp(?a39.*(d ? e39).ˆ2 ? bt39.*(t ? y39).ˆ2)) ...
 + sum(n42 .* Del.ˆb42 .* d .* Psi);

 phir d = sum(n7 .* d7 .* d.ˆ(d7?1) .* t.ˆt7) ...
 + sum(n34 .* d.ˆ(d34?1) .* t.ˆt34 .* ...
 exp(?d.ˆc34) .* (d34 ? c34.* d.ˆc34)) ...
 + sum(n39 .* d.ˆd39 .* t.ˆt39 .* ...
 exp(?a39.*(d ? e39).ˆ2 ? bt39.*(t ? y39).ˆ2) .* ...
 (d39./d ? 2.*a39.*(d?e39))) ...

 + sum(n42 .* (Del.ˆb42 .* (Psi + d.*Psi d) + ...
 b42 .* Del.ˆ(b42?1) .* Del d .* d .* Psi));

 phir dd = sum(n7 .* d7 .* (d7 ? 1) .* d.ˆ(d7?2) .* t.ˆt7) ...
 + sum(n34 .* exp(?d.ˆc34) .* d.ˆ(d34?2) .* t.ˆt34 .* ...
 ((d34 ? c34.* d.ˆc34).*(d34 ?1?c34.* d.ˆc34) ? c34.ˆ2 .* ...
d.ˆc34)) ...
 + sum(n39 .* t.ˆt39 .* ...
 exp(?a39.*(d ? e39).ˆ2 ? bt39.*(t ? y39).ˆ2) .* ...
 (?2.*a39.*d.ˆd39 + 4.*a39.ˆ2.*d.ˆd39.*(d?e39).ˆ2 ? ...
*d39.*a39.*d.ˆ(d39?1).*(d?e39) + d39.*(d39?1).*d.ˆ(d39?2))) ...
 + sum(n42 .* (Del.ˆb42 .* (2.*Psi d + d*Psi dd) + ...
*b42 .* Del.ˆ(b42?1) .* Del d .*(Psi + d.*Psi d) + ...
 b42.*d.*Psi.*...
 (Del.ˆ(b42?1).*Del dd + (b42?1).*Del.ˆ(b42?2).*Del d.ˆ2)));

 phir t = sum(n7 .* t7 .* d.ˆd7 .* t.ˆ(t7?1)) ...
 + sum(n34 .* d.ˆd34 .* t34 .* t.ˆ(t34?1) .* exp(?d.ˆc34)) ...
 + sum(n39 .* d.ˆd39 .* t.ˆt39 .* ...
 exp(?a39.*(d ? e39).ˆ2 ? bt39.*(t ? y39).ˆ2) .* ...
 (t39./t ? 2.*bt39.*(t?y39))) ...
 + sum(n42 .*d .* (?2.*tta.*b42.*Del.ˆ(b42?1).*Psi + ...
 Del.ˆb42.*Psi t));

 phir tt = sum(n7 .* t7 .* (t7?1) .* d.ˆd7 .* t.ˆ(t7?2)) ...
 + sum(n34 .* t34 .* (t34?1) .* d.ˆd34 .* t.ˆ(t34?2) .* ...
exp(?d.ˆc34)) ...
 + sum(n39 .* d.ˆd39 .* t.ˆt39 .* ...
 exp(?a39.*(d ? e39).ˆ2 ? bt39.*(t ? y39).ˆ2) .* ...
 ((t39./t ? 2.*bt39.*(t?y39)).ˆ2 ? t39./(t.ˆ2)?2.*bt39)) ...
 + sum(n42 .*d .* (Psi.*(2.*b42.*Del.ˆ(b42?1) + ...
*tta.ˆ2.*b42.*(b42?1).*Del.ˆ(b42?2)) ? ...
*tta.*b42.*Del.ˆ(b42?1).*Psi t + Del.ˆb42.*Psi tt));

 phir dt = sum(n7 .* t7 .* d7 .* d.ˆ(d7?1) .* t.ˆ(t7?1)) ...

 + sum(n34 .* exp(?d.ˆc34) .* t34 .* d.ˆ(d34?1) .* t.ˆ(t34?1) .* ...
 (d34 ? c34.* d.ˆc34)) ...
 + sum(n39 .* d.ˆd39 .* t.ˆt39 .* ...
 exp(?a39.*(d ? e39).ˆ2 ? bt39.*(t ? y39).ˆ2) .* ...
 (t39./t ? 2.*bt39.*(t?y39)).*(d39./d ? 2.*a39.*(d?e39))) ...
 + sum(n42.* ( Del.ˆb42.*(Psi t + d.*Psi dt) + ...
 d.*b42.*Del.ˆ(b42?1).*Del d.*Psi t ? ...
*tta.*b42.*Del.ˆ(b42?1).*(Psi + d.*Psi d) + ...
 d.*Psi.*(?A42.*b42.*0.5./bt42.*Del.ˆ(b42?1).*(d?1).*((d?1).ˆ2).ˆ(0.5./bt42?1) ...
? ...
*tta.*b42.*(b42?1).*Del.ˆ(b42?2).*Del d)));
...
08792];
 ...
 phio t = ao3(2) + ao3(3)/t + sum(ao8.*ttao8.*( (1?exp(?ttao8.*t)).ˆ(?1) ...
?1 ));

 phio tt = ?ao3(3)/(tˆ2) ? sum(ao8 .* ttao8.ˆ2 .* exp(?ttao8.*t) .* ...
 ( 1?exp(?ttao8.*t) ).ˆ(?2) );
 ...
 %??????????????????????????????????????????????????????????????????????????
 % Pressure
 P = 1000*d*rhoc*R*Tc/t*(1+d*phir d);
 % Entropy
 s = R*(t*(phio t + phir t) ? phio ? phir);
 % Internal energy
 u = R*Tc/t*t*(phio t+phir t);
 % Isochoric heat capacity
 cv = ?R*tˆ2*(phio tt+phir tt);
 % Isobaric heat capacity
 cp = R*(?tˆ2*(phio tt+phir tt) + ...
 (1+d*phir d?d*t*phir dt)ˆ2/(1+2*d*phir d+dˆ2*phir dd));
 % Enthalpy
 h = R*Tc/t*(1+t*(phio t+phir t)+d*phir d);
 % Speed of sound
 c = sqrt( R*Tc/t*1000*( 1 + 2*d*phir d + dˆ2*phir dd ? ...
 ((1 + d*phir d ? d*t*phir dt)ˆ2/(tˆ2*(phio tt+phir tt)))) );
 % Joule?Thompson coefficient
 mu = ?1/(R*d*rhoc)*(d*phir d + dˆ2*phir dd + d*t*phir dt) ...
 / ((1+d*phir d?d*t*phir dt)ˆ2 ? ...
tˆ2*(phio tt+phir tt)*(1+2*d*phir d+dˆ2*phir dd));
 ...
 %=========================================================================
 u NIST = 506.8006990081582;
 s NIST = 2.739101354918014;
 h NIST = 506.77604151182834;
